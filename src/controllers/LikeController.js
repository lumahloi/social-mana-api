                ...
                if (postCheck && userCheck) {
                    ...
                    if (!likeCheck.length) {
                        ...
                        if (!dislikeCheck.length) {
                            await connection.execute("INSERT INTO likes (postid, userid) VALUES (?, ?)", [postid, userid]);
            ...
            try {
                ...
                if (userCheck && postCheck) {
                    ...
                    if (unlike.length > 0 && unlike[0].userid === userid) {
                        const deletePromise = connection.execute("DELETE FROM likes WHERE postid = ? AND userid = ?", [postid, userid]);
